    ...
    >
      <div className="grid gap-4 md:grid-cols-2 lg:gap-5">
        {capabilities.items.map((item) => (
          <article
            key={item.title}
            className="rounded-2xl border border-edge bg-surface-2/60 p-5 transition-colors duration-300 hover:border-edge-strong hover:bg-surface-3/70 sm:rounded-3xl sm:p-7"
          >
            ...
            <ul className="mt-4 flex flex-col gap-2">
              {item.points.map((point) => (
                <li key={point} className="flex gap-2.5 text-sm leading-relaxed text-hero-sub">
                  <Tick />
                  {point}
                </li>
              ))}
